...
## JUST NU
Status: Batch 20 klar — Email capability live
Senast: read_inbox (Gmail+Outlook, metadata-only — subject/body/raw addresses blockerade på normaliser-nivå, bara from_domain överlever) + draft_email (skapar pending EmailDraft, SHA-256-fingerprints, mall-validering) + send_approved_draft (vägrar köra om status != approved) + DraftsPage.tsx /drafts med ✅/❌ + /api/drafts/{id}/approve|reject med fingerprint-cross-check + ApprovalRecord audit. Agenter skickar ALDRIG automatiskt — alltid mänsklig godkännande. 13 nya tester, 1161 totalt.
Nästa: Din fråga efter Batch 20

Last updated: 2026-05-26 (after Batch 20)
...
- Backend: **1161 tests passing** (V1 baseline 939, then per-batch additions through Batch 20; full per-batch accounting in the dated entries below), V1 feature-complete
